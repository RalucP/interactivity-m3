...
const settings = Object.freeze({
  ...
  console: /** @type HTMLElement */(document.getElementById('console'))
});
 ...
 * x: number
 * y: number
 * leftWristDistance: number
 * rightWristDistance: number
 * middlePoint: number
 * poseId: string
 * }} Distance
 */
...
const computeDistanceBetweenWrists = (pose) => {
  ...
  const leftWristDistance = Points.distance(leftWrist, centroid);
  const rightWristDistance = Points.distance(rightWrist, centroid);
  const middlePoint = (leftWristDistance + rightWristDistance) / 2;
...
const drawHead = (context, distance) => {
  const { scaleBy } = state;
  const { poses, squareDimension, scaleRadius } = settings;
  ...
  settings.console.innerHTML = `${poses.size}`;

  const leftRadius = distance.leftWristDistance * scaleRadius;
  const rightRadius = distance.rightWristDistance * scaleRadius;
  ...
  context.roundRect(0, 0, squareDimension, squareDimension, [leftRadius, rightRadius, rightRadius, leftRadius]);
  context.fill();
